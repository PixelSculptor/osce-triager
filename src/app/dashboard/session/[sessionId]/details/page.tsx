  ...
  return (
    <main className={styles.main}>
      ...
      <div className={styles.meta}>
        ...
        <span>Czas: {formatDuration(details.startedAt, details.completedAt)}</span>
      </div>
      ...
      {details.events.length === 0 ? (
        ...
        <>
          <p className={styles.orderHint}>↓ kolejność zlecania (od pierwszego do ostatniego)</p>
          <ol className={styles.eventList}>
            {details.events.map((event, index) => (
              <li key={`${event.testId}-${event.selectedAt.getTime()}`} className={styles.eventItem}>
