...
|---|-------------------------|--------|------------|--------------------------------|
| 1 | Student selects the correct life-saving test; validator returns "critical error" instead. Session outcome is wrong with no crash or visible signal. | High | Medium | PRD Business Logic (deterministic validator guardrail); interview Q1; interview Q3 (returns-proper-feedback = most-uncertain area); hot-spot dir `src/modules/session` — 7 changes/30 days |
...
|------|-----------------------------|----------------|--------------------------------------|-----------------------|-----------------------|
| #1 | Given scenario X + test Y (known classification), validator returns the exact expected status — not just non-null, but the correct value. Session outcome matches. | "Validator returned non-null, therefore correct" — it can return a wrong non-null value silently. | How does the validator read classification (DB query vs in-memory lookup)? Function signature, inputs, error shape? | Unit/integration on the validator function with fixture classifications | Testing only happy-path (correct test → positive); skipping the "correct test, validator fires critical error" case. Mirror-implementation oracle: never derive expected value from the validator itself. |
...
|---|------------|-----------------|---------------|------------|--------|---------------|
| 1 | Runner bootstrap + validator unit tests | Install vitest; prove first test passes; unit-test validator classification logic with fixture data | #1 | unit, integration | change opened | context/changes/testing-runner-bootstrap |
