...
- Dane sesji jednego studenta nie są dostępne z konta innego studenta.
...
Wejścia reguły: wybór badania diagnostycznego przez studenta w kontekście aktywnego scenariusza klinicznego. Każde badanie ma przypisaną klasyfikację: krytyczne (ratuje życie), optymalne (pierwsze z wyboru), akceptowalne, zbędne. Wyjście reguły: status oceny wyboru (poprawne / nieoptymalnie / błąd krytyczny). Użytkownik napotyka regułę po każdym kliknięciu — natychmiastowa informacja zwrotna po każdym wyborze.
...
- Niezalogowany użytkownik nie ma dostępu do żadnych scenariuszy ani historii sesji
...
- **Brak integracji z zewnętrznymi systemami medycznymi** — API z wytycznymi klinicznymi i systemy elektronicznych rekordów medycznych. Poza MVP.
...
- **Brak natywnej aplikacji mobilnej / desktopowej** — tylko responsywna aplikacja webowa. Natywne aplikacje mobilne i desktopowe są poza zakresem.
