...
Ta umiejętność jest trzecim ogniwem w łańcuchu rozruchowym (`/10x-shape → /10x-prd → 10x-tech-stack-selector → /10x-bootstrapper`). Jej jedyne zadanie: przekształcić napisany PRD w zalecany starter i małe, czytelne maszynowo przekazanie, które `/10x-bootstrapper` może odczytać, aby utworzyć szkielet projektu.

Umiejętność ta jest **ułatwiaczem decyzji w oparciu o wyselekcjonowany rejestr**, a nie generatorem rekomendacji od podstaw. Odczytuje priorytety PRD, zadaje maksymalnie ~6 pytań uzupełniających na ścieżce niestandardowej (lub skraca do sprawdzonej rekomendacji na ścieżce standardowej), analizuje karty startowe uwzględniające język w `references/starter-registry.yaml` i stosuje cztery filtry jakości. Bogate uzasadnienie pozostaje w rozmowie; przekazanie pliku jest minimalne.
...
- `/10x-bootstrapper` — konsument w dół łańcucha. Odczytuje frontmatter `context/foundation/tech-stack.md` i rejestr; tworzy szkielet projektu.
...
Jeśli plik jest **nieobecny**, wykonaj dokładnie to i ZATRZYMAJ — bez wywiadu awaryjnego, bez wbudowanego mini-PRD, bez odczytywania historii rozmów w poszukiwaniu zastępczych priorytetów:
...
- question: "context/foundation/tech-stack.md już istnieje. Jak chcesz postąpić?"
  ...
  - label: "Zapisz jako tech-stack-v2.md"
    description: "Zachowaj historię. Nowy wybór zostanie zapisany w następnym dostępnym slocie tech-stack-vN.md."
  - label: "Przerwij"
    description: "Wyjdź bez zapisu. Uzasadnienie rozmowy zostanie zachowane tylko na czacie."
  multiSelect: false

Zalecaną wartością domyślną jest tutaj "Nadpisz", ponieważ selektor stosu technologicznego to jednorazowa decyzja dla każdego projektu; wiele wersji jest zazwyczaj oznaką, że użytkownik ponownie rozważa, w którym to przypadku utrata poprzedniego wyboru jest zamierzona. Zapis wersji jest wyjściem awaryjnym.
...
3. **Ścieżka standardowa vs niestandardowa jest wiążąca.** Standardowa skraca do rekomendacji + Q4/Q5/nazwa projektu. Niestandardowa uruchamia pełne przejście plus autotest Q8. Nie mieszaj ich — ścieżka wybrana przez użytkownika w Q0 jest tym, co rejestruje `hints.path_taken`.
