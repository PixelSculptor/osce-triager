...
- **Test independence.** Agents happily generate tests where test B assumes test
  ...
  test.
- **Wait for state, not time.** "Never wait for timeout in production. Tests
  that wait for time are inherently flaky." Use `expect(locator).toBeVisible()`,
...
- **Risk-tied assertions.** The test name should bind it unambiguously to a risk
  ...
  `test('flashcard data persists after page reload', ...)`, not
  `test('test 1', ...)`.
